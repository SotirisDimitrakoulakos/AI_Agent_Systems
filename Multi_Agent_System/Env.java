...
public class Env extends Environment {
    ...
    class MarsModel extends GridWorldModel {
        ...
        class center_thread extends Thread {
          public void run() {
              taxiCenter();
          }
            ...
            while(true){
              if (!customers.isEmpty()){
                for (int[][] customer : customers){
                  if (flag==1){
                    Literal custID = Literal.parseLiteral("cust("+customer[0][0]+","customer[0][1]+")", "0");
                    addPercept(custID);
                  }
                  else if (flag==-1){
                    Literal custID = Literal.parseLiteral("cust("+customer[0][0]+","customer[0][1]+")", "1");
                    addPercept(custID);
                  }
                  mng_flag = -mng_flag;
                }
              ...
              try {
                Thread.sleep(700); // Sleep for 200 milliseconds
